...
end.date = as.POSIXct("2021-12-31")

# By using an index that is the logical AND of two vectors
c3 = complete_portfolio[start.date <= index(complete_portfolio) & index(complete_portfolio) <= end.date]
w33ortfolios

colnames(cleaned_portfolio3) <- c('Crypto','Treasury','NASon_return)


asset_allocation_mod1 = c('Crypto'= 0,'Treasury'= 0.4, 'NASDAQ' = 0.4, 'Gold' = 0.2)

asset_allocation_1_mod_return = Return.portfolio(cleaned_portfolio3, weights = asset_allocation_mod1,
                                                 rebalance_on = 'months')
...
asset_allocation_agg1 = c('Crypto'= 0,'Treasury'= 0.3, 'NASDAQ' = 0.65, 'Gold' = 0.05)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````O 'NASD Ratio of the Portfolio is')
...
asset_allocation_4_con_return = Return.portfolio(cleaned_portfolio3, weights = asset_allocation_con4,
                                                 rebalance_on = 'months')
...
asset_allocation_4_mod_return = Return.portfolio(cleaned_portfolio3, weights = asset_allocation_mod4,
                                                 rebalance_on = 'months')
...
asset_allocation_4_agg_return = Return.portfolio(cleaned_portfolio3, weights = asset_allocation_agg4,
                                                 rebalance_on = 'months')
...
Asset_Allocation_summary3 = table.AnnualizedReturns(asset_allocation_returns_all, Rf = 0.00000214)
Asset_Allocation_summary3_df = data.table::as.data.table(Asset_Allocation_summary3) 
...
table_ratios = table.AnnualizedReturns(com, Rf = 0.00000214)
table_ratios_df3 = data.table::as.data.table(Asset_Allocation_summary3) 
...
multi_fit_garch = multifit(garch_m_spec, cleaned_portfolio3)
multi_fit_garch
...
fit1 = dccfit(spec1, data = cleaned_portfolio3, fit.control = list(eval.se = TRUE),
              fit = multi_fit_garch)
