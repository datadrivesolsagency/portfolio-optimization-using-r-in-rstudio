...
start.date = as.POSIXct("2019-01-01")
end.date = as.POSIXct("2019-12-31")

# By using an index that is the logical AND of two vectors
c2 = complete_portfolio[start.date <= index(complete_portfolio) & index(complete_portfolio) <= end.date]

portfolio_sem_cleaned2 = na.locf(na.locf(c2), fromLast = FALSE)
sum(is.na(portfolio_sem_cleaned2))



portfolio_sem_cleaned2[is.na(portfolio_sem_cleaned2)] = 0
cleaned_portfolio2 = portfolio_sem_cleaned2
sum(is.na(cleaned_portfolio2))

#ASsigning column names to portfolios

colnames(cleaned_portfolio2) <- c('crypto','Treasury','NASDAQ','Gold')
...
asset_allocation_1_con_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_con1,
                                                 rebalance_on = 'months')
...
asset_allocation_1_mod_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_mod1,
                                                 rebalance_on = 'months')
...
asset_allocation_1_agg_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_agg1,
                                                 rebalance_on = 'months')
...
asset_allocation_2_con_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_con2,
                                                 rebalance_on = 'months')
...
asset_allocation_2_mod_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_mod2,
                                                 rebalance_on = 'months')
...
asset_allocation_2_agg_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_agg2,
                                                 rebalance_on = 'months')
...
asset_allocation_3_con_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_con3,
                                                 rebalance_on = 'months')
...
asset_allocation_3_mod_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_mod3,
                                                 rebalance_on = 'months')
...
asset_allocation_3_agg_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_agg3,
                                                 rebalance_on = 'months')
...
asset_allocation_4_con_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_con4,
                                                 rebalance_on = 'months')
...
asset_allocation_4_mod_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_mod4,
                                                 rebalance_on = 'months')
...
asset_allocation_4_agg_return = Return.portfolio(cleaned_portfolio2, weights = asset_allocation_agg4,
                                                 rebalance_on = 'months')
...
Asset_Allocation_summary2 = table.AnnualizedReturns(asset_allocation_returns_all, Rf = 0.0000775)
Asset_Allocation_summary2
Asset_Allocation_summary2_df = data.table::as.data.table(Asset_Allocation_summary2)

#----------------------Portfolio Optimization for this period---------------------------

#Optimization portfolio using random

funds = colnames(cleaned_portfolio2)
...
maxSrpot2 <- optimize.portfolio(R = cleaned_portfolio2, portfolio = first_port,
...
maxSrpot.roi <- optimize.portfolio(R = cleaned_portfolio2, portfolio = first_port1,
...
port_return.roi = Return.portfolio(cleaned_portfolio2, weights = wegiths.roi)
...
table_ratios = table.AnnualizedReturns(com)

table_ratios_df2 = data.table::as.data.table(table_ratios, Rf = 0.0000775)
